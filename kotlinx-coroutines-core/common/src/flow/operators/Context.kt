 ...
 * always fused so that only one properly configured channel is used for execution. Explicitly
 * specified buffer capacity takes precedence over `buffer()` or `buffer(Channel.BUFFERED)` calls,
 * which effectively requests a buffer of any size.
 *
